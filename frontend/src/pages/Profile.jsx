...
const handleUserUpdate=async ()=>{
  setUpdated(false)
  try{
    const res=await axios.put("http://localhost:4000/api/users/"+user._id,{username,email,password},{withCredentials:true})
...
const handleUserDelete=async()=>{
  try{
    const res=await axios.delete("http://localhost:4000/api/users/"+user._id,{withCredentials:true})
    setUser(null)
    navigate("/")
    // console.log(res.data)
...
const fetchUserPosts=async ()=>{
  try{
    const res=await axios.get("http://localhost:4000/api/posts/user/"+user._id)
    // console.log(res.data)
    setPosts(res.data)
